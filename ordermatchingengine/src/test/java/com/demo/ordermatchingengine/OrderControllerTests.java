...
public class OrderControllerTests {
    ...
    @Test
    public void testcase01_whenCreateOrder_thenOk() throws Exception {
    ...
    public void testcase02_whenGetAllOrders_thenOk() throws Exception{
    	this.mockMvc.perform(get("/getAllOrders"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(CONTENT_TYPE))
        .andExpect(jsonPath("$", hasSize(10)))
        .andExpect(jsonPath("$[0].price").value(100.0))
        .andExpect(jsonPath("$[1].price").value(100.5))
        ;
    }
    
    @Test
    public void testcase03_whenGetAllMatchedOrders_thenOk() throws Exception{
    	this.mockMvc.perform(get("/getAllMatchedOrders"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(CONTENT_TYPE))
    ...
    @Test
    public void testcase04_whenGetAllNotMatchedOrdersFromOrderBook_thenOk() throws Exception{
    	this.mockMvc.perform(get("/getAllOrders"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(CONTENT_TYPE))
        .andExpect(jsonPath("$", hasSize(3)))
        .andExpect(jsonPath("$[0].price").value(100.0))
        .andExpect(jsonPath("$[1].price").value(100.3))
        .andExpect(jsonPath("$[2].price").value(105.0))
        ;
    }
    
    @Test
    public void testcase05_whenDeleteOrder_thenOk() throws Exception{
    	this.mockMvc.perform(delete("/deleteOrder/1"))
        .andExpect(status().isOk())
        ;
    }
    
    @Test
    public void testcase06_whenUpdateOrderThen_Ok() throws Exception{
    	String orderUpdateJson = "{\"orderNumber\":8,\"orderType\":\"LIMIT\",\"transactionType\":\"SELL\",\"price\":106.0,\"quantity\":10,\"status\":\"UNMACTHED\"}";
        this.mockMvc.perform(post("/updateOrder")
        .contentType(CONTENT_TYPE)
   ...
   @Test
    public void testcase07_whenGetALlOrdersAgain_thenOk() throws Exception{
    	this.mockMvc.perform(get("/getAllOrders"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(CONTENT_TYPE))
        .andExpect(jsonPath("$", hasSize(2)))
        .andExpect(jsonPath("$[1].price").value(106.0));
    }
}
